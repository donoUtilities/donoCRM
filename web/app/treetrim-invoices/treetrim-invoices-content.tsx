...
interface TreeTrimInvoice {
  _id: string;
  legacy_id: string;
  invoiceNumber: string;
  date: string;
  bspdORDtap: string;
  treeTrimItems: string[];
  treeTrimItemsCount: number;
  team: string;
  isPaid: boolean;
  createdOn: string;
}

const columns: ColumnDef<TreeTrimInvoice>[] = [
  { label: "Invoice #", key: "invoiceNumber" },
  { label: "Date", key: "date", type: "date" },
  { label: "BSPD or DTAP", key: "bspdORDtap" },
  { label: "Tree Trim Items", key: "treeTrimItemsCount", type: "number" },
  { label: "Team", key: "team" },
  { label: "Payment Status", key: "isPaid" },
  { label: "Created On", key: "createdOn", type: "date" },
];

export function TreeTrimInvoicesContent() {
  const { data: session } = useSession();
  const [syncing, setSyncing] = React.useState(false);
  const { records, loading, loadingMore, totalCount, hasMore, loadMore, filterOptions, reset } =
    useInfiniteData<TreeTrimInvoice>({ apiUrl: "/api/treetrim-invoices" });
  ...
  async function handleSync() {
    setSyncing(true);
    try {
      const res = await fetch("/api/treetrim-invoices/sync", {
        method: "POST",
      });
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "Failed to sync Tree Trim invoices");
      }
      toast.success(data.message || "Tree Trim invoices synced successfully");
      reset();
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <PageHeader
        title="Tree Trim Invoices"
        description="All Tree Trim invoices"
        search={
      ...
      <DataTable
        ...
        emptyMessage={searchQuery || hasActiveFilters ? "No matching invoices" : "No tree trim invoices found"}
        renderCell={(row, col) => {
          if (col.key === "isPaid") {
            return (
          ...
          if (col.key === "treeTrimItemsCount") {
            const items = row.treeTrimItems || [];
