...
export const GET = withAuth(async (request) => {
  try {
    ...
    console.error("Failed to fetch Splicing Prices:", error);
    return NextResponse.json(
      { error: "Failed to fetch Splicing Prices" },
      { status: 500 }
    );
  }
});
...
export const POST = withAuth(async (request) => {
  try {
    ...
    console.error("Failed to create Splicing Price:", error);
    return NextResponse.json(
      { error: "Failed to create Splicing Price" },
      { status: 500 }
    );
  }
});
