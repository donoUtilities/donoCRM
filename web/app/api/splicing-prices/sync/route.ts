...
export const POST = withAuth(async () => {
  try {
    ...
    if (!response.ok) {
      ...
      console.error("AppSheet API returned error for SplicingPrices:", errorText);
    ...
    if (!Array.isArray(rows)) {
      console.error("Invalid response format from AppSheet for SplicingPrices:", rows);
      return NextResponse.json(
    ...
    return NextResponse.json({
      message: `Splicing Prices synced successfully. Total rows processed: ${rows.length}`,
      count: rows.length,
    });
  } catch (error) {
    console.error("Error syncing Splicing Prices:", error);
    return NextResponse.json(
