...
export const POST = withAuth(async () => {
  try {
    ...
    if (!response.ok) {
      ...
      console.error("AppSheet API returned error for BSPDPrices:", errorText);
    ...
    if (!Array.isArray(rows)) {
      console.error("Invalid response format from AppSheet for BSPDPrices:", rows);
      return NextResponse.json(
    ...
    for (const row of rows) {
      ...
      const isTickmark = row.Tickmark === "Y" || row.Tickmark === "true" || row.Tickmark === true || row.Tickmark === "Y/N";
      const isRedLine = row["Red Line"] === "Y" || row["Red Line"] === "true" || row["Red Line"] === true;
      const isPicture = row.Picture === "Y" || row.Picture === "true" || row.Picture === true;
    ...
    return NextResponse.json({
      message: `BSPD Prices synced successfully. Total rows processed: ${rows.length}`,
      count: rows.length,
    });
  } catch (error) {
    console.error("Error syncing BSPD Prices:", error);
    return NextResponse.json(
