...
export const POST = withAuth(async () => {
  try {
    ...
    const tableName = "TreeTrimPrices";
    ...
    if (!response.ok) {
      ...
      console.error("AppSheet API returned error for TreeTrimPrices:", errorText);
    ...
    if (!Array.isArray(rows)) {
      console.error("Invalid response format from AppSheet for TreeTrimPrices:", rows);
      return NextResponse.json(
    ...
    return NextResponse.json({
      message: `Tree Trim Prices synced successfully. Total rows processed: ${rows.length}`,
      count: rows.length,
    });
  } catch (error) {
    console.error("Error syncing Tree Trim Prices:", error);
    return NextResponse.json(
