...
export const GET = withAuth(async (request) => {
  try {
    ...
    const [docs, totalCount, teams, wireCenters, bspds, dtaps] = await Promise.all([
    ...
    const serialized = docs.map((d) => ({
      ...
      totalFeet: d.totalFeet ?? null,
      beforePicture: d.beforePicture || "",
      fiberFusionFibererial: d.fiberFusionFibererial || "",
      afterPicture: d.afterPicture || "",
      redLinePicture: d.redLinePicture || "",
    ...
    console.error("Failed to fetch Tree Trim Records:", error);
    return NextResponse.json(
      { error: "Failed to fetch Tree Trim Records" },
      { status: 500 }
    );
  }
});
