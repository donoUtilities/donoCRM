...
const columns: ColumnDef<SplicingPrice>[] = [
  { label: "ID", key: "id" },
...
export function SplicingPricesContent() {
  ...
  async function handleSync() {
    setSyncing(true);
    try {
      const res = await fetch("/api/splicing-prices/sync", {
        method: "POST",
      });
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "Failed to sync Splicing prices");
      }
      toast.success(data.message || "Splicing prices synced successfully");
      reset();
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <PageHeader
        title="Splicing Prices"
        description="Manage splicing pricing items"
        search={
      ...
      <DataTable
        ...
        emptyMessage={searchQuery || hasActiveFilters ? "No matching prices" : "No splicing prices found"}
      />
    </div>
  );
}
